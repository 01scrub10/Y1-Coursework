...
NumDroids = 3
NumDrones = 3
es = ecofactory(robots = NumRobots, droids = NumDroids, drones = NumDrones, chargers = [[40,20]], pizzas = 9)
es._max_weight = 120
    ...
    if bot.station is None:       
      ...
      for charger in chargers:
        ...
        if currentChargerDist < opportune_distance and charger.status == 'vacant' and bot.soc / bot.max_soc < 0.9: ##OPPORTUNE CHARGING - if we are close to a charger, charge there no matter battery level and skip loop
          bot.charge(charger)  
          break
        if currentChargerDist < minDist and charger.status =='vacant': ##find closest available charger 
          minDist = currentChargerDist
          closest_charger = charger
      #VARIABLE CHARGE THRESHOLD - charge threshold changes based on the distance and power of the robot, so that it always has enough charge to reach the charger and will not charge when not optimal
      charge_threshold = ((minDist / bot.max_speed * energy_consumption(bot.weight, bot.max_speed, bot.volitant))/bot.max_soc) + 0.1 # safety margin of 0.1, so the bot doesnt get too close to dying
      
      if closest_charger != None and bot.soc / bot.max_soc < charge_threshold: ##if still not charged (opportune charging didnt occur) and there is an available charger, charge at the closest
